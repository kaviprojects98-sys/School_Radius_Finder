import { onAuthStateChanged } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";
import { auth } from "./auth.js";


onAuthStateChanged(auth, (user) => {
    if (!user) window.location.href = "login.html";
});

let map;
let studentMarker;
let schoolMarker;
let circle;

let studentPos;
let schoolPos;

let placesService;
let schoolMarkers = [];

let excludedPlaceIds = [];

let schoolLabels = [];

let targetSchoolPlaceId = null;

// Schools are highlighted/searched within this much EXTRA distance
// beyond the exact student↔school circle (in km). 0.5 = 500 meters.
const SEARCH_RADIUS_BUFFER_KM = 0.5;

let customerNumber = "";

let studentGender = "";
let schoolNamesVisible = true;

// ---------------- CUSTOM SCHOOL LIST ----------------
const customSchoolList = [
    { id: 0,  lat: 6.994635174825783, lng: 80.01746318079749, name: "Udupila Primary School" },
    { id: 1,  lat: 6.983518221103399, lng: 80.02132481888104, name: "Kanduboda Primary School" },
    { id: 2,  lat: 6.980351873877105, lng: 80.01050431518179, name: "Siyambalapewatta Primary School" },
    { id: 3,  lat: 6.98949511714975,  lng: 80.00486878025154, name: "Sri Rajasinghe Primary School" },
    { id: 4,  lat: 6.968105821646406, lng: 80.00117916449963, name: "Yatihena Government School" },
    { id: 5,  lat: 6.997718576182054, lng: 79.98856174154979, name: "SKK Suriarachchi Primary School" },
    { id: 6,  lat: 6.98246890464621,  lng: 79.98580407180006, name: "Daranagama Primary School" },
    { id: 7,  lat: 6.963888468971635, lng: 79.98774284179764, name: "Kandewatta Siri Sumana Vidyalaya" },
    { id: 8,  lat: 6.96699292385619,  lng: 79.9782754398691,  name: "Sri Sangamiththa Vidyalaya" },
    { id: 9,  lat: 7.000526097123138, lng: 79.95337446706961, name: "Baptist Primary Vidyalaya" },
    { id: 10, lat: 7.0022602670911525, lng: 79.958393180104,  name: "Puwakwetiya Vidyalaya" },
    { id: 11, lat: 6.99814461154881,  lng: 79.96942271901023, name: "Pemananda Vidyalaya" },
    { id: 12, lat: 6.97149999123592,  lng: 79.954849107753,   name: "Sapugaskanda Vishaka Balika Vidyalaya", girlsOnly: true },
    { id: 13, lat: 6.974357120489106, lng: 79.95293204566329, name: "Sapugaskanda Sobitha Vidyalaya" },

    // ---- new schools ----
    { id: 14, lat: 7.031307443444814, lng: 80.0066586795863,  name: "Vidyadeepa Maha Vidyalaya Weboda North" },
    { id: 15, lat: 7.016376929421298, lng: 79.96900272119677, name: "Kirillawala Primary School" },
    { id: 16, lat: 7.016685702520541, lng: 80.02492247967925, name: "Mihindu Kanishta Vidyalaya" },
    { id: 17, lat: 7.002526678375252, lng: 79.94458114071996, name: "Mahabodi Primary School" },
    { id: 18, lat: 7.031748818798701, lng: 80.03127939619003, name: "Madduma Bandara Primary School" },
    { id: 19, lat: 6.959018729238134, lng: 79.96631926363955, name: "Mahinda Maha Vidyalaya" },
    { id: 20, lat: 6.963158462636454, lng: 80.00944903400611, name: "Subashi Rajakiya Vidyalaya" },
    { id: 21, lat: 6.94467668405531,  lng: 79.9481958088862,  name: "Bollegala Maha Vidyalaya" },
    { id: 22, lat: 7.290571987667287, lng: 80.64442037263571, name: "Mahamaya Primary School", girlsOnly: true },
    { id: 23, lat: 6.9809510985514,   lng: 79.92797495120664, name: "Viharamahadevi Balika Vidyalaya", girlsOnly: true },
    { id: 24, lat: 6.975964825886117, lng: 79.92594007711477, name: "D. A. Rajapaksha Vidyalaya" }
];

// ---------------- INIT MAP ----------------
function initMap() {

    map = new google.maps.Map(document.getElementById("map"), {
        center: { lat: 6.9271, lng: 79.8612 },
        zoom: 12
    });

    placesService = new google.maps.places.PlacesService(map);

    // Barcode/QR scanner listens for input as soon as the page is ready.
    setupBarcodeScanner();
}

window.initMap = initMap;

// ---------------- GEOCODE ----------------
function geocode(address, callback) {

    const geocoder = new google.maps.Geocoder();

    geocoder.geocode({ address }, (results, status) => {

        if (status === "OK") {
            const loc = results[0].geometry.location;

            callback({
                lat: loc.lat(),
                lng: loc.lng()
            });
        } else {
            alert("Location not found");
        }
    });
}

// ---------------- STUDENT (by address) ----------------
function searchStudent() {

    const address = document.getElementById("studentAddress").value;

    geocode(address, (pos) => {
        placeStudentMarker(pos);
    });
}

// ---------------- STUDENT (by shared link) ----------------
// NEW FEATURE: lets you paste a link a customer sent (e.g. a Google Maps
// link) and jump straight to that exact location.
function locateFromLink() {

    const linkInput = document.getElementById("studentLink").value.trim();

    if (!linkInput) {
        alert("Paste a location link first");
        return;
    }

    const coords = extractCoordsFromLink(linkInput);

    if (!coords) {

        if (/goo\.gl|maps\.app\.goo\.gl/i.test(linkInput)) {
            // Shortened links don't contain the coordinates themselves —
            // the real coordinates only appear after Google's server
            // redirects to the full URL, which client-side JS can't follow
            // across origins. Ask the user to open it once and paste the
            // resulting full URL instead.
            alert(
                "This looks like a shortened Google Maps link (goo.gl / maps.app.goo.gl).\n\n" +
                "Please open it once in your browser, then copy the FULL address-bar URL " +
                "(it will contain something like '@6.9271,79.8612') and paste that here instead."
            );
        } else {
            alert(
                "Couldn't find coordinates in that link.\n\n" +
                "Please paste a Google Maps link that contains the location " +
                "(it should include '@lat,lng' or '?q=lat,lng'), or just paste plain 'lat,lng' coordinates."
            );
        }
        return;
    }

    placeStudentMarker(coords);
    showCustomPopup("📍 Student location found from link");
}

// Tries several known Google Maps URL patterns to pull out lat/lng.
// Returns { lat, lng } or null if nothing matched.
function extractCoordsFromLink(text) {

    // Pattern 1: .../@6.9271,79.8612,17z/...  (standard "place" links)
    let match = text.match(/@(-?\d+\.\d+),\s*(-?\d+\.\d+)/);
    if (match) {
        return { lat: parseFloat(match[1]), lng: parseFloat(match[2]) };
    }

    // Pattern 2: ...?q=6.9271,79.8612  or  &q=6.9271,79.8612
    match = text.match(/[?&]q=(-?\d+\.\d+),\s*(-?\d+\.\d+)/);
    if (match) {
        return { lat: parseFloat(match[1]), lng: parseFloat(match[2]) };
    }

    // Pattern 3: !3d6.9271!4d79.8612  (embedded coords in some place URLs)
    match = text.match(/!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/);
    if (match) {
        return { lat: parseFloat(match[1]), lng: parseFloat(match[2]) };
    }

    // Pattern 4: plain "6.9271, 79.8612" text (no URL at all)
    match = text.match(/^\s*(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)\s*$/);
    if (match) {
        return { lat: parseFloat(match[1]), lng: parseFloat(match[2]) };
    }

    return null;
}

// Shared helper: drops/replaces the student marker at a given position.
// Used by both searchStudent() and locateFromLink().
function placeStudentMarker(pos) {

    studentPos = pos;

    if (studentMarker) studentMarker.setMap(null);

    studentMarker = new google.maps.Marker({
        position: pos,
        map,
        label: {
            text: "STUDENT",
            color: "black",
            fontWeight: "bold"
        },
        draggable: true,
        zIndex: 1000
    });

    map.setCenter(pos);
    map.setZoom(15);
}

// ---------------- SCHOOL AUTOCOMPLETE (from custom list) ----------------
function filterSchoolDropdown() {

    const input = document.getElementById("schoolAddress");
    const dropdown = document.getElementById("schoolDropdown");
    const query = input.value.trim().toLowerCase();

    if (!query) {
        dropdown.style.display = "none";
        dropdown.innerHTML = "";
        return;
    }

    const matches = customSchoolList.filter(school =>
        school.name.toLowerCase().includes(query)
    );

    if (matches.length === 0) {
        dropdown.style.display = "none";
        dropdown.innerHTML = "";
        return;
    }

    dropdown.innerHTML = matches
        .map(school => `<div onclick="selectSchoolFromDropdown(${school.id})">${school.name}</div>`)
        .join("");

    dropdown.style.display = "block";
}

function selectSchoolFromDropdown(schoolId) {

    const school = customSchoolList.find(s => s.id === schoolId);
    if (!school) return;

    document.getElementById("schoolAddress").value = school.name;
    document.getElementById("schoolDropdown").style.display = "none";

    schoolPos = { lat: school.lat, lng: school.lng };

    // Remember which school is the TARGET school so we can exclude it
    // from the "schools found inside the circle" results.
    targetSchoolPlaceId = school.id;

    if (schoolMarker) schoolMarker.setMap(null);

    schoolMarker = new google.maps.Marker({
        position: schoolPos,
        map,
        icon: "https://maps.google.com/mapfiles/ms/icons/red-dot.png",
        label: {
            text: school.name,
            color: "black",
            fontWeight: "bold"
        },
        draggable: true,
        zIndex: 999
    });

    map.setCenter(schoolPos);
    map.setZoom(15);
}

document.addEventListener("click", function (e) {
    const wrapper = document.getElementById("schoolAddress").parentElement;
    if (!wrapper.contains(e.target)) {
        document.getElementById("schoolDropdown").style.display = "none";
    }
});

// ---------------- CONFIRM POSITIONS ----------------
function confirmStudent() {
    studentPos = {
        lat: studentMarker.getPosition().lat(),
        lng: studentMarker.getPosition().lng()
    };

    showCustomPopup("✅ Student position confirmed");
}

function confirmSchool() {
    schoolPos = {
        lat: schoolMarker.getPosition().lat(),
        lng: schoolMarker.getPosition().lng()
    };

    showCustomPopup("✅ School position confirmed");
}

// ---------------- DISTANCE ----------------
function getDistance(lat1, lng1, lat2, lng2) {

    const R = 6371;

    const dLat = (lat2 - lat1) * Math.PI / 180;
    const dLng = (lng2 - lng1) * Math.PI / 180;

    const a =
        Math.sin(dLat/2) * Math.sin(dLat/2) +
        Math.cos(lat1*Math.PI/180) *
        Math.cos(lat2*Math.PI/180) *
        Math.sin(dLng/2) * Math.sin(dLng/2);

    return 2 * R * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
}

// ---------------- ANALYSIS ----------------
function analyze() {

    if (!studentPos || !schoolPos) {
        alert("Confirm both positions first");
        return;
    }

    customerNumber = document.getElementById("customerNumber").value;
    
    studentGender = document.getElementById("studentGender").value;

    schoolMarkers.forEach(m => m.setMap(null));
    schoolMarkers = [];

    schoolLabels.forEach(label => label.setMap(null));
    schoolLabels = [];

    if (circle) circle.setMap(null);

    const radiusKm = getDistance(
        studentPos.lat,
        studentPos.lng,
        schoolPos.lat,
        schoolPos.lng
    );

    // draw circle — light blue fill, thin sharp black border
    circle = new google.maps.Circle({
        map,
        center: studentPos,
        radius: radiusKm * 1000,
        strokeColor: "#000000",
        strokeOpacity: 1,
        strokeWeight: 1,
        fillColor: "#66b3ff",
        fillOpacity: 0.12
    });

    studentMarker.setZIndex(1000);
    schoolMarker.setZIndex(999);

    // The blue circle stays at the exact distance, but schools are
    // highlighted using a bigger search radius (exact distance + buffer).
    filterCustomSchools(radiusKm, radiusKm + SEARCH_RADIUS_BUFFER_KM);
}

// ---------------- FILTER CUSTOM SCHOOL LIST BY DISTANCE ----------------
function filterCustomSchools(exactRadiusKm, searchRadiusKm) {

    // FIX: exclusion + gender rules are now applied ONCE, up front,
    // before splitting into the buffered/exact-radius lists. Previously
    // these rules were only applied to `visibleSchools` (used for the
    // count/name list), while the green markers drawn on the map were
    // built from `schoolsToHighlight`, which never had gender/exclusion
    // applied — so an excluded or gender-filtered school's marker still
    // showed up on the map even though it correctly disappeared from the
    // count and the names list.
    //
    // Gender rule: when "boy" is selected, hide any school flagged
    // girlsOnly: true in customSchoolList (currently: Sapugaskanda
    // Vishaka Balika Vidyalaya, Mahamaya Primary School, Viharamahadevi
    // Balika Vidyalaya).
    const eligibleSchools = customSchoolList.filter(school => {

        if (school.id === targetSchoolPlaceId) return false;
        if (excludedPlaceIds.includes(school.id)) return false;

        if (studentGender === "boy" && school.girlsOnly) return false;

        return true;
    });

    // Schools to highlight (inside circle + 500m buffer)
    const schoolsToHighlight = eligibleSchools.filter(school => {

        const distance = getDistance(
            studentPos.lat,
            studentPos.lng,
            school.lat,
            school.lng
        );

        return distance <= searchRadiusKm;
    });

    // Schools to count and list (inside exact circle only)
    const visibleSchools = schoolsToHighlight.filter(school => {

        const distance = getDistance(
            studentPos.lat,
            studentPos.lng,
            school.lat,
            school.lng
        );

        return distance <= exactRadiusKm;
    });

  
    document.getElementById("schoolCountCircle").textContent = visibleSchools.length;
    document.getElementById("distanceInfo").innerHTML =
        "<b>Distance:</b> " + exactRadiusKm.toFixed(2) + " km" +
        
        
        "<br><b>Customer Number:</b> " + (customerNumber || "--") +
        "<br><b>Student Gender:</b> " + (studentGender ? (studentGender === "boy" ? "Boy" : "Girl") : "--") +
        "<br><b>Student Position:</b> " + studentPos.lat + ", " + studentPos.lng;

    const namesListHtml = visibleSchools.length
        ? "<ul style='margin:4px 0 0 18px; padding:0;'>" +
          visibleSchools.map(s => "<li>" + s.name + "</li>").join("") +
          "</ul>"
        : "<i>No schools in range</i>";

    document.getElementById("schoolNamesList").innerHTML = namesListHtml;
    updateSchoolNamesDisplay();

    schoolsToHighlight.forEach(school => {

        const position = { lat: school.lat, lng: school.lng };

        const marker = new google.maps.Marker({
            position: position,
            map: map,
            icon: "https://maps.google.com/mapfiles/ms/icons/green-dot.png",
            zIndex: 500
        });

        marker.addListener("click", () => {

            const remove = confirm(
                `Remove "${school.name}" from the results?`
            );

            if (remove) {

                excludedPlaceIds.push(school.id);

                analyze();
            }
        });

        schoolMarkers.push(marker);

        const label = new google.maps.OverlayView();

        label.onAdd = function () {

            const div = document.createElement("div");
            div.style.position = "absolute";
            div.style.background = "white";
            div.style.padding = "2px 5px";
            div.style.border = "1px solid black";
            div.style.fontSize = "12px";
            div.style.fontWeight = "bold";
            div.style.whiteSpace = "nowrap";

            div.innerHTML = school.name;

            this.div = div;

            const panes = this.getPanes();
            panes.overlayLayer.appendChild(div);
        };

        label.draw = function () {

            const projection = this.getProjection();
            const pixelPosition = projection.fromLatLngToDivPixel(
                new google.maps.LatLng(position.lat, position.lng)
            );

            if (this.div) {
                this.div.style.position = "absolute";
                this.div.style.left = pixelPosition.x + "px";
                this.div.style.top = pixelPosition.y + "px";
            }
        };

        label.onRemove = function () {
            if (this.div) {
                this.div.parentNode.removeChild(this.div);
                this.div = null;
            }
        };

        label.setMap(map);
        schoolLabels.push(label);
    });
}

// ---------------- SHOW/HIDE SCHOOL NAMES LIST ----------------
function toggleSchoolNames() {
    schoolNamesVisible = !schoolNamesVisible;
    updateSchoolNamesDisplay();
}

function updateSchoolNamesDisplay() {
    const box = document.getElementById("schoolNamesList");
    const btn = document.getElementById("toggleNamesBtn");

    box.style.display = schoolNamesVisible ? "block" : "none";
    btn.innerText = schoolNamesVisible ? "Hide School Names" : "Show School Names";
}

function printMap() {
    window.print();
}

// ---------------- PRINT: STABLE, CENTERED, NEVER CROPPED ----------------
// Padding reserved around the fitted circle. "right" is bigger to make
// room for the distance box, so the circle visually centers in the
// remaining left-hand space instead of being centered on the whole page.
const PRINT_FIT_PADDING = { top: 20, right: 300, bottom: 20, left: 20 };

function focusCircleForPrint() {

    if (!circle || !studentPos) {
        // nothing to fit yet — just print whatever is currently shown
        window.print();
        return;
    }

    // 1. Switch the page into its final print layout FIRST.
    //    This changes #map to its full-page print size.
    document.body.classList.add("print-mode");

    // 2. Wait two animation frames so the browser has actually painted
    //    the new layout before we touch the map — this is required for
    //    Google Maps to measure the container correctly.
    requestAnimationFrame(() => {
        requestAnimationFrame(() => {

            // Tell Google Maps its container size changed.
            google.maps.event.trigger(map, "resize");

            // 3. Now fit the circle to the REAL print-sized container.
            const bounds = circle.getBounds();
            map.fitBounds(bounds, PRINT_FIT_PADDING);

            // 4. Only print once the map has finished settling on the
            //    new bounds/zoom — this is what keeps the circle stable
            //    and fully inside the page every time.
            google.maps.event.addListenerOnce(map, "idle", () => {
                setTimeout(() => {
                    window.print();
                }, 300);
            });
        });
    });
}

// 5. After the print dialog closes, restore the normal on-screen view.
window.addEventListener("afterprint", () => {

    document.body.classList.remove("print-mode");

    requestAnimationFrame(() => {
        requestAnimationFrame(() => {
            google.maps.event.trigger(map, "resize");

            if (circle) {
                map.fitBounds(circle.getBounds());
            } else if (studentPos) {
                map.setCenter(studentPos);
            }
        });
    });
});

function showCustomPopup(message) {
    document.getElementById("customPopupMessage").innerHTML = message;
    document.getElementById("customPopupOverlay").style.display = "block";
}

function closeCustomPopup() {
    document.getElementById("customPopupOverlay").style.display = "none";
    focusScanInput();
}

// =====================================================================
// ---------------- BARCODE / QR SCANNER SUPPORT ----------------------
// =====================================================================
//
// TWO ways to feed a scan into the app now:
//
// 1. HARDWARE SCANNER — acts like a keyboard: it "types" the encoded
//    text very fast into whatever field is focused, then sends an
//    Enter key. Handled by setupBarcodeScanner() / #barcodeScanInput
//    below, unchanged from before.
//
// 2. PHONE CAMERA — uses the html5-qrcode library (loaded in the HTML
//    <head>/<body> via a <script> tag) to open the phone's back camera,
//    detect a QR code, and decode it. Handled by startCameraScan() /
//    stopCameraScan() further down. Requires HTTPS (or localhost) —
//    browsers block camera access on plain HTTP.
//
// Both paths end up calling the SAME handleScannedData(text) function,
// so the parsing/filling/analysis logic only exists in one place.
//
// Expected data format (generated by the companion QR generator page):
//
//   STUDENT_LAT,STUDENT_LNG|SCHOOL_ID|CATEGORY|CUSTOMER_NUMBER|GENDER
//
// Example:
//   6.994635,80.017463|3|category2|CN-10234|boy
//
// - STUDENT_LAT,STUDENT_LNG : decimal coordinates of the student
// - SCHOOL_ID                : the numeric "id" from customSchoolList
// - CATEGORY                 : category1 / category2 / category3
// - CUSTOMER_NUMBER           : any text/number
// - GENDER                    : boy / girl
//
// A dedicated, always-focused input field (#barcodeScanInput) captures
// hardware scans. This is far more reliable than trying to guess "fast
// typing" timing on the whole page, and it does not interfere with
// normal typing in the other fields.

function setupBarcodeScanner() {

    const scanInput = document.getElementById("barcodeScanInput");
    if (!scanInput) return;

    scanInput.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
            e.preventDefault();
            const value = scanInput.value;
            scanInput.value = "";
            if (value.trim()) {
                handleScannedData(value);
            }
        }
    });

    // Re-focus the scan field whenever the user clicks somewhere that
    // isn't itself an interactive form element, so the very next scan
    // is captured automatically without having to click back into it.
    document.addEventListener("click", (e) => {
        const tag = e.target.tagName;
        const isInteractive =
            tag === "INPUT" || tag === "SELECT" || tag === "BUTTON" || tag === "TEXTAREA" ||
            e.target.closest("#schoolDropdown");
        if (!isInteractive) {
            focusScanInput();
        }
    });

    focusScanInput();
}

function focusScanInput() {
    const scanInput = document.getElementById("barcodeScanInput");
    if (scanInput) scanInput.focus();
}

function handleScannedData(rawText) {

    const text = rawText.trim();
    const parts = text.split("|");

    if (parts.length < 5) {
        showCustomPopup("⚠️ Barcode not recognized. Expected 5 fields separated by '|'.");
        return;
    }

    const [latLngPart, schoolIdPart, categoryPart, customerNumberPart, genderPart] = parts;

    const latLngMatch = latLngPart.trim().match(/^(-?\d+(?:\.\d+)?)\s*,\s*(-?\d+(?:\.\d+)?)$/);

    if (!latLngMatch) {
        showCustomPopup("⚠️ Barcode error: student coordinates not readable.");
        return;
    }

    const lat = parseFloat(latLngMatch[1]);
    const lng = parseFloat(latLngMatch[2]);

    // 1. Place & confirm the student
    placeStudentMarker({ lat, lng });
    confirmStudent();
    document.getElementById("studentAddress").value = "";
    document.getElementById("studentLink").value = latLngPart.trim();

    // 2. Select & confirm the school
    const schoolId = parseInt(schoolIdPart.trim(), 10);
    const school = customSchoolList.find(s => s.id === schoolId);

    if (!school) {
        showCustomPopup("⚠️ Barcode error: school id '" + schoolIdPart + "' not found in list.");
        return;
    }

    selectSchoolFromDropdown(school.id);
    confirmSchool();

    // 3. Fill the remaining fields
    const categoryValue = categoryPart.trim();
    const categorySelect = document.getElementById("customerCategory");
    if ([...categorySelect.options].some(o => o.value === categoryValue)) {
        categorySelect.value = categoryValue;
    }

    document.getElementById("customerNumber").value = customerNumberPart.trim();

    const genderValue = genderPart.trim().toLowerCase();
    const genderSelect = document.getElementById("studentGender");
    if ([...genderSelect.options].some(o => o.value === genderValue)) {
        genderSelect.value = genderValue;
    }

    // 4. Run the analysis automatically
    analyze();

    showCustomPopup("✅ Scanned and filled: " + school.name);
    focusScanInput();
}

// =====================================================================
// ---------------- CAMERA (PHONE) QR SCANNER SUPPORT -----------------
// =====================================================================
//
// Uses the html5-qrcode library (must be loaded via <script> tag in the
// HTML, e.g. https://unpkg.com/html5-qrcode) to open the device's back
// camera and decode a QR code in real time. Needs HTTPS (or localhost)
// because browsers only grant camera access on secure origins.

let html5QrCode = null;

function startCameraScan() {

    const modal = document.getElementById("cameraScanModal");
    if (!modal) {
        showCustomPopup("⚠️ Camera scan UI not found on this page.");
        return;
    }

    if (typeof Html5Qrcode === "undefined") {
        showCustomPopup("⚠️ Camera scanner library failed to load. Check your internet connection.");
        return;
    }

    modal.style.display = "block";

    html5QrCode = new Html5Qrcode("qrReader");

    html5QrCode.start(
        { facingMode: "environment" },   // prefer the back camera on phones
        { fps: 10, qrbox: 250 },
        (decodedText) => {
            // Stop the camera as soon as we get a hit, then reuse the
            // exact same handler the hardware scanner uses.
            stopCameraScan();
            handleScannedData(decodedText);
        },
        () => {
            // fires continuously while no QR code is in frame — ignore
        }
    ).catch((err) => {
        showCustomPopup("⚠️ Could not start camera: " + err);
        stopCameraScan();
    });
}

function stopCameraScan() {

    const modal = document.getElementById("cameraScanModal");
    if (modal) modal.style.display = "none";

    if (html5QrCode) {
        html5QrCode.stop()
            .then(() => html5QrCode.clear())
            .catch(() => { /* already stopped/cleared, ignore */ })
            .finally(() => {
                html5QrCode = null;
                focusScanInput();
            });
    } else {
        focusScanInput();
    }
}

// ---------------- RESET ----------------
function resetForm() {

    document.getElementById("studentAddress").value = "";
    document.getElementById("studentLink").value = "";
    document.getElementById("schoolAddress").value = "";
    document.getElementById("schoolDropdown").innerHTML = "";
    document.getElementById("schoolDropdown").style.display = "none";
    
    document.getElementById("customerNumber").value = "";
    document.getElementById("studentGender").value = "";
    document.getElementById("schoolCountCircle").textContent = "0";
    if (studentMarker) { studentMarker.setMap(null); studentMarker = null; }
    if (schoolMarker) { schoolMarker.setMap(null); schoolMarker = null; }
    if (circle) { circle.setMap(null); circle = null; }

    schoolMarkers.forEach(m => m.setMap(null));
    schoolMarkers = [];

    schoolLabels.forEach(label => label.setMap(null));
    schoolLabels = [];

    studentPos = undefined;
    schoolPos = undefined;
    targetSchoolPlaceId = null;
    excludedPlaceIds = [];
    customerNumber = "";
    
    studentGender = "";

    document.getElementById("distanceInfo").innerHTML = "Distance: --";
    document.getElementById("schoolNamesList").innerHTML = "";
    schoolNamesVisible = true;
    updateSchoolNamesDisplay();

    if (map) {
        map.setCenter({ lat: 6.9271, lng: 79.8612 });
        map.setZoom(12);
    }

    focusScanInput();
}

// Im doing this becaus of the onclick in html. I need to add this for every single function I declear

window.searchStudent = searchStudent;
window.locateFromLink = locateFromLink;
window.filterSchoolDropdown = filterSchoolDropdown;
window.selectSchoolFromDropdown = selectSchoolFromDropdown;
window.confirmStudent = confirmStudent;
window.confirmSchool = confirmSchool;
window.analyze = analyze;
window.focusCircleForPrint = focusCircleForPrint;
window.toggleSchoolNames = toggleSchoolNames;
window.closeCustomPopup = closeCustomPopup;
window.printMap = printMap;
window.resetForm = resetForm;
window.startCameraScan = startCameraScan;
window.stopCameraScan = stopCameraScan;
